...
W2_ = normrnd(0, sqrt(2/(hidden_layer1_size + hidden_layer2_size)),...
    [hidden_layer1_size + 1, hidden_layer2_size]);
W3_ = normrnd(0, sqrt(2/(hidden_layer2_size + output_layer_size)),...
    [hidden_layer2_size + 1, output_layer_size]);

%% Training
start_idx = 1;
end_idx = start_idx + minibatch_size - 1;
for epoch = 1 : N_epoch
    d_W3_  = zeros(size(W3_));
    ...
    dropout_mask_hidden_layer2 = [(rand([1,hidden_layer2_size]) <= dropout_keeprate_hidden_2) 1];
    ...
    for i = start_idx:end_idx % not so correct
        ...
        out_2 = o_1_hat * W2_;
        o_2 = sigmoid(out_2);
        o_2_hat = horzcat(o_2, ones(size(o_2, 1), 1));
        
        o_2_hat = o_2_hat .* dropout_mask_hidden_layer2;
        out_3 = o_2_hat * W3_;
        o_3 = sigmoid(out_3);         
                
        % Back-prop
        e = o_3 - training_data_Y(i,:);
    ...
    t_o_2_hat = horzcat(t_o_2, ones(size(t_o_2, 1), 1));
    
    W3_compensated = [W3_(1:end-1,:) * dropout_keeprate_hidden_2; W3_(end,:)];
    t_out_3 = t_o_2_hat * W3_compensated;
    t_o_3 = sigmoid(t_out_3);
    
    [tmp, digit_out]=max(t_o_3,[],2);
